...
export default function generateStaffHTMLReport(
  staff: any,
  groupedDuties: any,
  reviews: any[]
) {
  ...
  const formatShiftType = (shiftType: string) => {
    ...
    if (shiftType.toLowerCase() === "both") return "12h/24h";
    return shiftType.toUpperCase();
  };

  const formatShiftTime = (shiftTime: string) => {
    if (!shiftTime) return "N/A";
    if (shiftTime.toLowerCase() === "both") return "Day/Night";
  ...
  if (nonEssentialServices) {
    ...
    const selectedMandatory = nonEssentialServices.mandatory.filter((duty: any) => staffNonEssentialServices.includes(duty.label));
    const selectedOptional = nonEssentialServices.optional.filter((duty: any) => staffNonEssentialServices.includes(duty.label));
    ...
    if (selectedMandatory.length > 0 || selectedOptional.length > 0) {
      ...
      [...selectedMandatory, ...selectedOptional].forEach((duty: any) => {
  ...
  Object.entries(groupedDuties).forEach(([category, group]: any) => {
    ...
    if (category === "non-essential services" || category === "Non-essential services" || category === "non essential services") return;
